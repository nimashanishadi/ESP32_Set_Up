...
static esp_err_t event_handler(void *ctx, system_event_t *event) {
    switch (event->event_id) {
        ...
        case SYSTEM_EVENT_STA_GOT_IP:
            ...
            xEventGroupSetBits(s_wifi_event_group, WIFI_CONNECTED_BIT);  // Signal connection established
            break;
        default:
...
void station_init() {
    ...
    esp_log_level_set("*", ESP_LOG_DEBUG);  // Set log level
    ...
    wifi_config.sta.channel = WIFI_CHANNEL; // Optional: Set channel explicitly

    ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_STA, &wifi_config));
    ESP_LOGI(TAG, "Wi-Fi configured for SSID: %s", ESP_WIFI_SSID);
    ...
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "Wi-Fi connect initiated successfully");
    } else {
        ESP_LOGE(TAG, "Wi-Fi connect failed, error code: 0x%x", ret);
    }
}
